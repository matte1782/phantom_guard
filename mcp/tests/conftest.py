...
def inject_hallucination_db(hallucination_db_stub, monkeypatch):
    ...
    Targets check_package (Day 2+), is_hallucinated (Day 3+), stage1 (Day 5+).
    """
    try:
        ...
        pass
    try:
        import phantom_guard_mcp.tools.is_hallucinated as ih_mod
        monkeypatch.setattr(ih_mod, "_hallucination_db", hallucination_db_stub)
    except (ImportError, AttributeError):
        pass
    try:
        import phantom_guard_mcp.evaluation.stage1 as s1_mod
        monkeypatch.setattr(s1_mod, "_hallucination_db", hallucination_db_stub)
    except (ImportError, AttributeError):
        pass
